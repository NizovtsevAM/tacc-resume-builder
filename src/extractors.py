...
class TechnologyExtractor:
    ...
    TECH_PATTERNS = [
        ...
        (
            r"(?i)(?<!\w)(PostgreSQL|Postgres|MySQL|SQL\s*Server|MongoDB|Redis|Elasticsearch|ElasticSearch|Cassandra|DynamoDB|Cosmos\s*DB|MSSQL|SQLite|MariaDB)(?!\w)",
            "Database",
        ),
        (
            r"(?i)(?<!\w)(React|Angular|Vue\.?js|Svelte|Next\.?js|Nuxt|Blazor|Webpack|Vite|Bootstrap|Tailwind|jQuery)(?!\w)",
            "Frontend",
        ),
        (
            r"(?i)(?<!\w)(\.NET\s*(Core|Framework)?|ASP\.NET|C#|VB\.NET|F#|Entity\s*Framework|LINQ|WCF|WPF|WinForms|\.Net|\.net)(?!\w)",
            "Backend",
        ),
        (
            r"(?i)(?<!\w)(Java|Spring\s*Boot|Spring|Kotlin|JVM|Hibernate|Maven|Gradle|JSP|Servlets)(?!\w)",
            "Backend",
        ),
        (r"(?i)(?<!\w)(Python|Django|Flask|FastAPI|Celery|NumPy|Pandas|Jupyter)(?!\w)", "Backend"),
        (
            r"(?i)(?<!\w)(Node\.?js|Express|Nest\.?js|TypeScript|JavaScript|Deno|Bun|npm|yarn)(?!\w)",
            "Backend",
        ),
        (
            r"(?i)(?<!\w)(Selenium|Playwright|Cypress|Puppeteer|Appium|TestCafe|xUnit|nUnit|Jest|Mocha|Chai|Jasmine|Karma|SpecFlow|Gauge|Postman|SoapUI|Swagger)(?!\w)",
            "Testing",
        ),
        (
            r"(?i)(?<!\w)(Airflow|Spark|Databricks|dbt|Snowflake|Kafka|Flink|Hadoop|Hive|Presto|Trino|Apache\s*Spark|Apache\s*Airflow)(?!\w)",
            "Data Engineering",
        ),
        (
            r"(?i)(?<!\w)(Golang|Rust|Scala|Swift|Ruby|PHP|Elixir|Clojure|Haskell|Perl)(?!\w)",
            "Backend",
        ),
        (
            r"(?i)(?<!\w)(CI/CD|Jenkins|GitLab\s*CI|GitHub\s*Actions|CircleCI|TeamCity|ArgoCD|TravisCI|Bamboo)(?!\w)",
            "DevOps",
        ),
        (
            r"(?i)(?<!\w)(RabbitMQ|SQS|SNS|ActiveMQ|NATS|ZeroMQ|Azure\s*Service\s*Bus)(?!\w)",
            "Backend",
        ),
        (r"(?i)(?<!\w)(gRPC|REST|GraphQL|WebSocket|SOAP|OData|SignalR)(?!\w)", "Backend"),
        (
            r"(?i)(?<!\w)(Microservices|DDD|CQRS|Event\s*Sourcing|Event\s*Driven|Clean\s*Architecture|SOLID|Onion\s*Architecture|Hexagonal\s*Architecture)(?!\w)",
            "Architecture",
        ),
        (
            r"(?i)(?<!\w)(React\s*Native|Flutter|Xamarin|MAUI|SwiftUI|UIKit|iOS|Android)(?!\w)",
            "Frontend",
        ),
        (
            r"(?i)(?<!\w)(Windows\s*Server|Linux|Ubuntu|CentOS|Debian|RedHat|Nginx|Apache|IIS)(?!\w)",
            "DevOps",
        ),
        (
            r"(?i)(?<!\w)(Git|SVN|TFS|Azure\s*DevOps|Jira|Confluence|Trello|Asana|Slack)(?!\w)",
            "DevOps",
        ),
    ]
